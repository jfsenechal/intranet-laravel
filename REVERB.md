...
> ```
>
> VAPID keys are generated by `php artisan webpush:vapid`. There is **no** `VITE_VAPID_PUBLIC_KEY` — the browser reads the public key from `config('webpush.vapid.public_key')`, rendered server-side into the page.
>
