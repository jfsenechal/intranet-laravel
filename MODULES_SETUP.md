...
The application uses a modular architecture where each module is a separate Composer package installed
from a local path repository (as a symlink). This document describes the structure, the conventions to
follow, and the workflow for creating a new module.

## Module Architecture

Modules live in `modules/` and follow a specific architectural pattern that separates concerns and keeps
...
**IMPORTANT:** For all Filament resources, we use separate classes for table and form configurations
instead of defining schemas directly in the Resource class.
...
1. **Separation of Concerns**: Each module is self-contained and each class has a single, well-defined responsibility
2. **Maintainability**: Easy to find and modify table or form configurations
3. **Reusability**: Modules (and their form/table configurations) can be reused elsewhere
4. **Clean Resource Classes**: The main Resource class remains minimal and readable
5. **Independent Testing**: Each module can have its own tests
6. **Version Control**: Modules can be versioned independently
7. **Team Development**: Different teams can work on different modules
8. **Organization**: Clear directory structure makes navigation intuitive
...
mkdir -p modules/ModuleName/src/{Providers/Filament,Filament/Pages,Filament/Widgets}
```
...
class ModuleNameServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        $this->mergeConfigFrom(
            __DIR__.'/../config/module-name.php',
            'module-name'
        );
    }
...
**File:** `modules/ModuleName/src/Providers/Filament/ModuleNamePanelProvider.php`
...
namespace AcMarche\ModuleName\Providers\Filament;
...
1. **PluginTrait**: Always use the `PluginTrait` to correctly resolve paths within the module.
   `$path` is relative to the provider file, so `src/Providers/Filament/` needs `'/../../'`
...
```json
{
    "extra": {
        "laravel": {
            "providers": [
                "AcMarche\\ModuleName\\ModuleNameServiceProvider",
                "AcMarche\\ModuleName\\Providers\\Filament\\ModuleNamePanelProvider"
            ]
        }
    }
}
...
php artisan vendor:publish --tag=module-name-migrations
```
...
For existing modules or when creating new modules from French database schemas, you **MUST** create a
migration to rename French columns to English equivalents.
...
- **Document** (`acmarche/document`) - Document management with file uploads, categories,
  publish/unpublish and soft deletes. Complete example with forms, tables and infolists
- **News** (`acmarche/news`) - News/blog management with rich text editor, featured images,
  SEO-friendly slugs, featured flag and soft deletes
...
- **Mediation** (`acmarche/mediation`) - Recent PanelProvider example (`src/Providers/Filament/`)
...
**Documents table**: `id`, `title`, `description`, `file_path`, `file_name`, `file_size`, `mime_type`,
`category`, `is_published`, `published_at`, timestamps, soft deletes

**News table**: `id`, `title`, `slug` (unique), `excerpt`, `content`, `featured_image`, `author`,
`category`, `is_published`, `is_featured`, `published_at`, timestamps, soft deletes
...
17. **Add factories and seeders** for each module model
18. **Add policies** for authorization where the module exposes sensitive data

## Support

For module-specific documentation, see the module's own `README.md`, e.g.:
- `modules/Document/README.md`
- `modules/News/README.md`

For Filament documentation:
- Use the `search-docs` MCP tool with a package filter for `filament/filament`
