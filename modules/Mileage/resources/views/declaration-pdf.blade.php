...
<div class="container">
    ...
    <table class="table table-bordered table-hover">
        <tr>
            ...
            <td>
                {{ $declaration->last_name | upper }} {{ $declaration->first_name }}<br/>
    ...
    <h4 class="text-success">Pour frais de déplacement :
        <strong>{{ $declarationSummary->totalRefund | number_format(2, ',', '.') }} €</strong></h4>
    ...
    <br/><br/>
    Certifié sincère et véritable à la somme de <strong>{{ $declarationSummary->totalRefund | number_format(2, ',', '.') }}
        €</strong>
    <br/><br/>
    ...
    <table class="table table-bordered table-hover">
        <tr>
            <td width="50%">
                Marche-en-Famenne, le {{ 'today' | date('d-m-Y') }}
            </td>
            <td>
                Signature:
            </td>
    ...
    Délibération du Collège Communal du {{ $declaration->college_date | date('d-m-Y') }}
    <br/><br/>
    ...
    {% if declaration.typeDeplacement == 'internal' %}
    {% include _internal_trips.blade.php' with {'rate': declaration.rate} %}
    {% elseif declaration.typeDeplacement == 'external' %}
    {% include _external_trips.blade.php' with {'rate': declaration.rate} %}
    {% endif %}
