...
new class extends Component
{
    ...
    {
        return Article::query()->orderBy('id')->first();
    }
    ...
    @if ($article)
        <a href="/articles/{{ $article->id }}" class="group relative flex flex-col gap-3 p-5 text-white">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-lg font-extrabold group-hover:underline">{{ $article->title }}</h2>
            </div>
            @if ($article->excerpt)
                <p class="text-sm opacity-90">{{ $article->excerpt }}</p>
            @endif
    ...
    @else
        <div class="relative flex flex-col gap-3 p-5 text-white">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-lg font-extrabold">Sport</h2>
            </div>
            <p class="text-sm opacity-90">Aucun article disponible.</p>
            <div class="flex items-center gap-3 text-3xl">
                <span class="animate-float">⚽</span>
                <span class="animate-float [animation-delay:300ms]">🏃</span>
                <span class="animate-float [animation-delay:600ms]">🏋️</span>
